"""
drive-sync.py — Sincroniza fotos desde Google Drive para ARTES BUHO MANAGEMENT
Cuenta: booking@artesbuhomanagement.com
Usa las credenciales OAuth2 del hub ARTES-BUHO_API-GOOGLE.
...
# Rutas — PENDIENTE: migrar credenciales al hub ARTES-BUHO_API-GOOGLE
BASE = Path(__file__).resolve().parent.parent
CREDS_DIR = Path("C:/Users/elrub/Desktop/CARPETA CODEX/01_PROYECTOS/ARTES-BUHO_API-GOOGLE")
...
def get_drive_service():
    ...
    # Auto-refresh token si esta expirado
...
def list_images(service, folder_id):
    ...
    while True:
        ...
        if not page_token:
            break
    return [f for f in results if f.get('mimeType', '').startswith('image/')]
...
def build_catalog():
    print("Conectando a Google Drive (booking@artesbuhomanagement.com)...")
    try:
        service = get_drive_service()
    except Exception as e:
        print(f"ERROR: No se pudo conectar a Google Drive: {e}")
        print("Verifica las credenciales en ARTES-BUHO_API-GOOGLE")
        return None
    ...
    catalog = {
        'version': 1,
        'account': 'booking@artesbuhomanagement.com',
        'directo': [],
        'estudio': [],
        'stats': {}
    }
